...
// ========== PATTERN DETECTIVE (FIXED & ROBUST) ==========
...
function generatePatternRound() {
    ...
    while (oddEmoji === baseEmoji) {
        oddEmoji = patternEmojis[Math.floor(Math.random() * patternEmojis.length)];
    }
...
function handlePatternClick(index, cell) {
    ...
    if (index === currentOddIndex) {
        ...
        document.getElementById('pattern-feedback').innerHTML = `<span class="text-red-500">Not quite... Byte will remember for next time!</span>`;
...
function nextPatternRound() {
    patternRound++;
    document.getElementById('pattern-round').innerText = patternRound;
...
function endPatternGame() {
    ...
    grid.innerHTML = `
        <div class="col-span-3 text-center py-8">
            ...
            <h4 class="text-3xl font-bold text-gray-900 mb-2">Amazing Detective Work!</h4>
            <p class="text-xl text-gray-600 mb-6">You scored <span class="font-extrabold text-amber-500">${patternScore}/5</span></p>
            
            <div class="max-w-xs mx-auto bg-amber-50 border border-amber-200 rounded-2xl p-4 text-left text-sm">
                <p class="font-semibold text-amber-700 mb-1">What Byte Learned:</p>
                <p class="text-amber-600">Real AI uses <strong>pattern recognition</strong> to find differences in images, just like you did! This is how self-driving cars spot traffic signs.</p>
            </div>
        </div>
    `;
    ...
    nextBtn.innerHTML = 'Play Again <i class="fa-solid fa-redo ml-2"></i>';
    nextBtn.onclick = function() {
        closeModal('pattern-modal');
        setTimeout(startPatternGame, 300);
    };
    ...
    content.innerHTML = `
        <div class="mb-6">
            <div class="flex items-center justify-between mb-4">
                <div><span class="font-bold text-purple-700">Training Item ${currentLabelIndex + 1} / 6</span></div>
                <div class="text-xs px-3 py-1 bg-purple-100 text-purple-600 rounded-full font-bold">Label this!</div>
            </div>
            
            <div class="flex flex-col items-center justify-center py-8 bg-gradient-to-br from-purple-50 to-white rounded-3xl border border-purple-100">
                <div class="text-[120px] mb-4 drop-shadow">${item.emoji}</div>
                <div class="text-4xl font-bold text-gray-800 mb-1">${item.name}</div>
                <div class="text-purple-500 text-sm">What category does this belong to?</div>
            </div>
        </div>
        
        <div class="grid grid-cols-3 gap-4">
            <button onclick="labelItem('Animal')" class="kid-button py-4 px-6 bg-gradient-to-br from-emerald-400 to-teal-500 text-white text-xl font-bold rounded-2xl flex flex-col items-center justify-center gap-y-1 shadow-lg">
                <span class="text-4xl">🐾</span>
                <span>Animal</span>
            </button>
            <button onclick="labelItem('Food')" class="kid-button py-4 px-6 bg-gradient-to-br from-orange-400 to-amber-500 text-white text-xl font-bold rounded-2xl flex flex-col items-center justify-center gap-y-1 shadow-lg">
                <span class="text-4xl">🍎</span>
                <span>Food</span>
            </button>
            <button onclick="labelItem('Vehicle')" class="kid-button py-4 px-6 bg-gradient-to-br from-sky-400 to-blue-500 text-white text-xl font-bold rounded-2xl flex flex-col items-center justify-center gap-y-1 shadow-lg">
                <span class="text-4xl">🚗</span>
                <span>Vehicle</span>
            </button>
        </div>
...
function finishTraining() {
    ...
    content.innerHTML = `
        <div class="text-center py-12">
            <div class="mx-auto w-20 h-20 border-4 border-purple-300 border-t-purple-600 rounded-full animate-spin mb-8"></div>
            <h4 class="text-3xl font-bold text-purple-700 mb-3">Byte is studying your labels!</h4>
            <p class="text-gray-600 max-w-xs mx-auto">Analyzing patterns in your training data...</p>
        </div>
    `;
...
function showTestItem() {
    ...
    const sameCategoryItems = Object.keys(trainingLabels).filter(name => {
        const trainedItem = labelItems.find(i => i.name === name);
        return trainedItem && trainedItem.category === item.category;
    });
    
    if (sameCategoryItems.length > 0) {
        const votes = {};
        sameCategoryItems.forEach(name => {
            const cat = trainingLabels[name];
    ...
    content.innerHTML = `
        <div class="mb-8">
            <div class="text-center">
                <div class="inline-block px-4 py-1 bg-purple-100 text-purple-700 text-xs font-bold rounded-full mb-4">NEW ITEM FOR BYTE</div>
                <div class="text-[110px] mb-3">${item.emoji}</div>
                <div class="text-4xl font-extrabold text-gray-900">${item.name}</div>
            </div>
        </div>
        
        <div class="mb-6">
            <p class="text-center text-sm font-semibold text-purple-600 mb-3">Byte thinks this is a...</p>
            <div class="flex justify-center">
                <div class="px-10 py-4 bg-gradient-to-r from-purple-100 to-violet-100 border-2 border-purple-300 rounded-3xl text-center">
                    <span class="text-5xl font-extrabold text-purple-700">${predicted}</span>
                </div>
            </div>
        </div>
        
        <div class="text-center">
            <p class="text-sm text-gray-500 mb-4">Was Byte correct? Help him learn!</p>
            <div class="flex gap-4 justify-center">
                <button onclick="evaluateLabel(true, '${predicted}')" class="px-8 py-3.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-2xl flex-1 max-w-[140px]">Yes! Correct</button>
                <button onclick="evaluateLabel(false, '${predicted}')" class="px-8 py-3.5 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-2xl flex-1 max-w-[140px]">No, it's ${item.category}</button>
            </div>
        </div>
    `;
...
function evaluateLabel(correct, predicted) {
    ...
    setTimeout(() => {
        currentLabelIndex++;
        if (currentLabelIndex < testItems.length) {
            showTestItem();
        } else {
            endLabelGame();
        }
...
function endLabelGame() {
    ...
    if (accuracy >= 75) {
        message = 'Byte is becoming a classification expert!';
        starsEarned = 5;
        createConfetti(35);
    } else if (accuracy >= 50) {
        message = 'Good job training him! More examples = better AI.';
        starsEarned = 4;
    } else {
        message = 'AI learns best with consistent labels. Try again!';
        starsEarned = 2;
    }
    ...
    content.innerHTML = `
        <div class="text-center py-6">
            <div class="text-6xl mb-4">${accuracy >= 75 ? '🎉' : '👏'}</div>
            <h4 class="text-4xl font-extrabold text-purple-700 mb-2">Training Complete!</h4>
            <div class="text-6xl font-black text-purple-600 mb-2">${accuracy}%</div>
            <p class="text-xl text-gray-600 mb-6">Byte's accuracy on new items</p>
            
            <div class="max-w-sm mx-auto bg-purple-50 border border-purple-200 p-5 rounded-3xl text-left">
                <p class="font-bold text-purple-700 mb-2">What you taught Byte:</p>
                <div class="text-sm text-purple-600 space-y-1">
                    ${Object.entries(trainingLabels).map(([name, cat]) => `<div class="flex justify-between"><span>${name}</span> <span class="font-mono">${cat}</span></div>`).join('')}
                </div>
            </div>
            
            <p class="mt-6 text-sm text-purple-600 italic">${message}</p>
        </div>
    `;
    
    const nextBtn = document.getElementById('label-next-btn');
    nextBtn.innerHTML = 'Play Again <i class="fa-solid fa-redo ml-2"></i>';
    nextBtn.onclick = () => {
        closeModal('label-modal');
        setTimeout(startLabelGame, 400);
    };
    nextBtn.classList.remove('hidden');
}

// Stub for any old onclick (prevents errors)
function labelNextStep() {
    // Not used in new flow - games auto-advance
    console.log('Label game auto-advances');
}
...
function showCurrentScenario() {
    ...
    if (currentScenarioIndex >= adventureScenarios.length) {
        endAdventure();
        return;
    }
    ...
    storyEl.innerHTML = `
        <div class="flex items-start gap-4">
            <div class="text-6xl flex-shrink-0 mt-1">🤖</div>
            <div class="flex-1">
                <p class="text-xl leading-tight text-gray-800">${scenario.text}</p>
            </div>
        </div>
    `;
    ...
    scenario.choices.forEach((choice, idx) => {
        ...
        btn.innerHTML = `
            <div class="text-4xl mt-0.5">${choice.emoji}</div>
            <div class="flex-1">
                <div class="font-bold text-lg text-gray-900">${choice.text}</div>
                <div class="text-xs text-pink-500 mt-1">${choice.reward > 0 ? '+' : ''}${choice.reward} happiness</div>
            </div>
        `;
...
function makeAdventureChoice(choice, scenario) {
    ...
    storyEl.innerHTML = `
        <div class="text-center py-4">
            <div class="text-5xl mb-3">${choice.emoji}</div>
            <p class="font-bold text-xl ${choice.reward > 0 ? 'text-emerald-600' : 'text-rose-500'}">${choice.reward > 0 ? 'Great choice!' : 'Hmm, Byte learned from that!'}</p>
            <p class="text-sm text-gray-500 mt-2">${choice.reward > 0 ? 'Byte feels happier and smarter.' : 'Every experience helps Byte grow.'}</p>
        </div>
    `;
...
function endAdventure() {
    ...
    if (adventureState.happiness > 85) {
        finalMessage = "Byte is overjoyed! You're an amazing trainer!";
        starsToAdd = 5;
        createConfetti(50);
    } else if (adventureState.happiness > 60) {
        starsToAdd = 4;
    }
    ...
    storyEl.innerHTML = `
        <div class="text-center py-6">
            ...
            <h4 class="text-3xl font-extrabold text-pink-700 mb-3">Adventure Complete!</h4>
            <p class="text-2xl font-bold text-gray-800 mb-2">Byte's Final Happiness: <span class="text-pink-600">${adventureState.happiness}</span></p>
            <p class="text-gray-600 mb-6">${finalMessage}</p>
            
            <div class="bg-white border border-pink-200 p-4 rounded-2xl text-left max-w-xs mx-auto">
                <p class="font-bold text-sm text-pink-600 mb-2">Byte learned these lessons:</p>
                <ul class="text-sm text-gray-600 space-y-1">
                    ${adventureState.history.map(h => `<li class="flex items-start gap-2"><span class="text-pink-400">•</span> <span>${h.choice}</span></li>`).join('')}
                </ul>
            </div>
        </div>
    `;
    ...
    choicesEl.innerHTML = `
        <button onclick="resetAdventure()" class="col-span-2 kid-button py-4 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-bold rounded-2xl flex items-center justify-center gap-x-3">
            <span>Play Adventure Again</span>
